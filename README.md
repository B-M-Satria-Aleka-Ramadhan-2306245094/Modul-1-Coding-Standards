...
## REFLECTION 4
# Refleksi TDD (Percival, 2017)
Penerapan alur Red–Green–Refactor membantu mengarahkan desain selama pengembangan. Spesifikasi kelas Order didorong oleh test, misalnya konstruktor harus menolak daftar products kosong, memiliki status default “WAITING_PAYMENT”, dan hanya menerima tiga status valid yaitu “WAITING_PAYMENT”, “SUCCESS”, dan “CANCELLED” seperti yang diuji pada OrderTest.java. Implementasi pada Order.java kemudian mengikuti perilaku yang diuji, bukan detail implementasi tertentu. Test juga berfokus pada perilaku sistem melalui pengecekan output dan exception, sehingga refactoring dapat dilakukan tanpa mengubah test.

Siklus umpan balik dari unit test cukup cepat karena sebagian besar tidak memerlukan boot Spring context. Hanya terdapat satu test integrasi yang memuat konteks aplikasi untuk memverifikasi konfigurasi. Seluruh test berhasil dijalankan melalui ./gradlew test, yang memberikan perlindungan dasar terhadap regresi. Ke depannya, kualitas TDD dapat ditingkatkan dengan menulis test gagal terlebih dahulu sebelum implementasi, menambah validasi pada atribut seperti id, author, dan orderTime, serta mempertimbangkan penggunaan enum OrderStatus agar lebih type-safe. Selain itu, pemisahan yang lebih jelas antara unit test dan integration test akan membantu menjaga kecepatan siklus pengembangan.

# Evaluasi Prinsip F.I.R.S.T.
Sebagian besar prinsip F.I.R.S.T. telah terpenuhi. Test tergolong Fast karena unit test berjalan cepat dengan stub atau implementasi in-memory, meskipun integration test yang memuat Spring context sedikit lebih lambat. Test juga Independent karena setiap test membuat objek baru tanpa berbagi state. Dari sisi Repeatable, test tidak bergantung pada jaringan, database, atau sistem file sehingga dapat dijalankan secara konsisten. Test bersifat Self-validating karena menggunakan asersi jelas seperti assertEquals dan assertThrows. Terakhir, prinsip Timely sebagian besar terpenuhi karena test ditulis sebelum implementasi final, meskipun disiplin menulis failing test terlebih dahulu masih bisa ditingkatkan.Sebagian besar prinsip F.I.R.S.T. telah terpenuhi. Test tergolong Fast karena unit test berjalan cepat dengan stub atau implementasi in-memory, meskipun integration test yang memuat Spring context sedikit lebih lambat. Test juga Independent karena setiap test membuat objek baru tanpa berbagi state. Dari sisi Repeatable, test tidak bergantung pada jaringan, database, atau sistem file sehingga dapat dijalankan secara konsisten. Test bersifat Self-validating karena menggunakan asersi jelas seperti assertEquals dan assertThrows. Terakhir, prinsip Timely sebagian besar terpenuhi karena test ditulis sebelum implementasi final, meskipun disiplin menulis failing test terlebih dahulu masih bisa ditingkatkan.
